...
async function seedUserRole (){
  try {
    let user = await User.findOne({
      'username':'Destany_Gleichner82',
      'isActive':true,
      'isDeleted':false
    });
    let userRole = await Role.findOne({ code: 'SYSTEM_USER' }, { id: 1 });
    if (user && user.isPasswordMatch('qT61HfClUrrWc5f') && userRole){
    ...
    let admin = await User.findOne({
      'username':'Freeman.Buckridge',
      'isActive':true,
      'isDeleted':false
    });
    let adminRole = await Role.findOne({ code: 'SYSTEM_USER' }, { id: 1 });
    if (admin && admin.isPasswordMatch('CNBSkTqjF16FamH') && adminRole){
      let count = await UserRole.countDocuments({
        userId: admin.id,
        roleId: adminRole.id
      });
...
async function seedUser () {
  try {
    ...
    if (!user || !user.isPasswordMatch('qT61HfClUrrWc5f') ) {
      let user = new User({
        'password':'qT61HfClUrrWc5f',
        'username':'Destany_Gleichner82',
    ...
    if (!admin || !admin.isPasswordMatch('CNBSkTqjF16FamH') ) {
      let admin = new User({
        'password':'CNBSkTqjF16FamH',
        'username':'Freeman.Buckridge',
