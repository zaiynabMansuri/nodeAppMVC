...
beforeAll(async function (){
  try {
    ...
    const db = client.db('DWSTG_test');

    const user = db.collection('user');
    inserted_user = await user.insertOne({
      username: 'Keshaun44',
      password: 'ZH9DYFjv68Ky8GS',
      email: 'Cale.Johnston@yahoo.com',
      name: 'Benjamin Terry',
      mobileNo: '(272) 488-3945 x2367',
      role: 905,
      resetPasswordLink: {},
      loginRetryLimit: 508,
      loginReactiveTime: '2022-02-17T17:48:37.159Z',
      id: '61ee33347bf9bb14e96e4220'
    });
  }
  catch (err) {
  ...
  test('should register a user', async () => {
    let registeredUser = await request(app)
      ...
      .send({
        ...
        'password':'nGONgnWWYAsg8Kx',
        'email':'Maybelle_Pfannerstill@hotmail.com',
        'name':'Miss Rachael Hessel',
        'mobileNo':'963.528.6578',
...
describe('POST /forgot-password -> if email passed from request body is valid and OTP sent successfully', () => {
  test('should return success message', async () => {
    ...
    let user = await request(app)
      .post('/admin/auth/forgot-password')
      .send({ 'email':'Maybelle_Pfannerstill@hotmail.com', });
